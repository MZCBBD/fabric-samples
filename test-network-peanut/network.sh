...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating did Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-did.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-badge.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  infoln "Generating CCP files for did and Org2"
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/did/msp organizations/fabric-ca/did/tls-cert.pem organizations/fabric-ca/did/ca-cert.pem organizations/fabric-ca/did/IssuerPublicKey organizations/fabric-ca/did/IssuerRevocationPublicKey organizations/fabric-ca/did/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/badge/msp organizations/fabric-ca/badge/tls-cert.pem organizations/fabric-ca/badge/ca-cert.pem organizations/fabric-ca/badge/IssuerPublicKey organizations/fabric-ca/badge/IssuerRevocationPublicKey organizations/fabric-ca/badge/fabric-ca-server.db'
